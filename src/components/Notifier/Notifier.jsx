...
// Helper function to get card image with secret handling
  ...
  return createPortal(
    <div className={`notifier-overlay ${isVisible ? 'visible' : 'hidden'}`} onClick={handleClose}>
      <div className="notifier-content" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="notifier-hint">Click outside to close • (Auto-closes in 3s)</div>
      </div>
    </div>,
    document.body
  );
...
export default function Notifier({ publicData, actualPlayerId, wsRef }) {
  ...
  const getSetName = (setId, playerId) => {
    ...
    if (set && (set.setName === "Tommy Beresford" || set.setName === "Tuppence Beresford")) {
