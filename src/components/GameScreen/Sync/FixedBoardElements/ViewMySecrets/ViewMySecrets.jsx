...
/**
 * @file ViewMySecrets.jsx
 * @description Floating button that opens a portal with the player's **private secrets**.
 *
 * === Canonical shapes (from API DOCUMENT) ===
 * @typedef {{ id:number, revealed:boolean, name:(string|null) }} PublicSecret
 *
 * === Props ===
 * @typedef {Object} ViewMySecretsProps
 * @property {PublicSecret[]} secrets - Current player's secrets (some may be hidden).
 *
 * Notes:
 * - Button shows a row of small lights indicating secrets and revealed state (green/cream vs dark).
 * - In modal: if `secret.revealed === false`, shows front image with a back overlay.
 */
                ...
                secrets.map((secret) => (
                  <div key={secret.id} className="my-secret-card">
                    {secret.revealed ? (
                      ...
                      <>
                        {/* Front plus a back overlay to indicate "hidden" */}
                        <img
                          className="front"
                          src={SECRETS_MAP[secret.name]}
                          alt={`Secret ${secret.name}`}
